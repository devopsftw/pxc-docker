...
RUN apt-get update && apt-get -y --no-install-recommends --no-install-suggests install ca-certificates qpress percona-xtradb-cluster-56 bind9-host

# install galera-healthcheck
ADD https://github.com/sttts/galera-healthcheck/releases/download/v20150303/galera-healthcheck_linux_amd64 /bin/galera-healthcheck
